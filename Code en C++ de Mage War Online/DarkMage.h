...
class DarkMage : public MonsterBase, public std::enable_shared_from_this<DarkMage>
{
private:
	...
	sf::Vector2f monster_size{ 29,40 };
	...
	sf::Clock clock_between_teleportation;
	sf::Vector2f position_possible[3]{ sf::Vector2f(0,0),sf::Vector2f(0,0) ,sf::Vector2f(0,0) };
	int source_x = 0;
	int rayon = 30;
	int level_monster = 1;
	bool drawed = false;
	...
	int id_monster;
	float timer_animation = 0.175;
	float cooldown_teleportation = 1;
	float range_teleportation = 300;
	float range_of_view = range_of_view_monster;// 800;
	float monster_type_fragment_given = StrongFragment;
	float monster_fragments_given = 15;
	float time_being_stunned = 0;
	float exp_given = 20;
	float money_given;
	float speed_projectile_throwing = 535;
	...
	void TeleportationGestion(sf::Vector2f position_player);
public:
	DarkMage();
	~DarkMage();
