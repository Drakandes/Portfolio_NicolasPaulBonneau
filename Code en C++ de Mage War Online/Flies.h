...
class Flies : public MonsterBase, public std::enable_shared_from_this<Flies>
{
private:
	...
	sf::Vector2f monster_size{ 50,50 };
	sf::Vector2f position_going_to;
	float speed_initial = 400;
	float speed;
	sf::Clock clock_blob_update;
	float monster_maximum_health = 676;
	float armor = 15;
	float reset_timer = 0.5;
	float attack_speed = 0.9;
	float monster_holder_health;
	float damage = 225;
	float time_clock_destination = 0.8;
	...
	int source_x = 0;
	int source_y;
	int range_of_view = range_of_view_monster;// 1000;
	int rayon = 30;
	int level_monster = 1;
