...
Bomby::Bomby()
{
	if (!monster_texture.loadFromFile("Bomby.png"))
	{
		std::cout << "Error: Couldn't load Bomby texture" << std::endl;
	}

	if (!shadow_texture.loadFromFile("BombyShadow.png"))
	{
...
void Bomby::Init(int id_monster_received, sf::Vector2f position_spawn_r, int id_spawn_r, float size_spawn_r, int level_monster_r, sf::Vector2f position_monster, bool is_elite, bool is_monster_guardian)
{
...
#pragma endregion Fragment
	...
	if (is_elite)
	{
		if (is_guardian)
		{
			monster_texture.loadFromFile("BombyGuardian.png");
		}
		else
		{
			monster_texture.loadFromFile("BombyElite.png");
		}
		MonsterEliteInit();
	...
	position_origin = position_monster;
	...
	string_monster_name.insert(string_monster_name.getSize(), sf::String("   Bomby"));
	if (monster_is_elite)
	{
		if (is_guardian)
		{
...
void Bomby::MonsterEliteGestion(sf::Vector2f playerPosition, float player_speed, sf::Vector2f player_position, sf::Vector2f velocity_player)
{
...
void Bomby::Health_maintenance()
{
	...
	if (monster_alive)
	{
		sf::Vector2f Bomby_position = sprite_monster.getPosition();
		monster_maximum_health_bar.setSize(sf::Vector2f(monster_size.x, 4));
		monster_maximum_health_bar.setPosition(sf::Vector2f(Bomby_position.x, Bomby_position.y - monster_size.y / 2 - 5));

		monster_health_bar.setSize(sf::Vector2f(monster_size.x * (monster_holder_health / monster_maximum_health), 4));
		monster_health_bar.setPosition(sf::Vector2f(Bomby_position.x, Bomby_position.y - monster_size.y / 2 - 5));

		text_monster_name.setPosition(sf::Vector2f(Bomby_position.x, Bomby_position.y - monster_size.y / 2 - 20));
	}
}

float Bomby::GetDurationOfStun()
{
	return stun_duration;
}

void Bomby::CheckEntityStillInMap()
{
...
void Bomby::Movement_gestion(float DELTATIME, sf::Vector2f player_position, sf::Vector2f velocity_player)
{
	if (!is_moving && GlobalFunction::GetDistanceBetweenTwoPositions(player_position, GetCurrentPosition()) <= range_of_view)
	{
		if (clock_switch_destination.getElapsedTime().asSeconds() >= time_clock_destination)
		{
			...
			{
				position.x = GlobalFunction::getRandomRange(200, 300);
			}
			else
			{
				position.x = -GlobalFunction::getRandomRange(200, 300);
			...
			if (GlobalFunction::getRandomRange(1, 1000) < 500)
			{
				position.y = GlobalFunction::getRandomRange(200, 300);
			}
			else
			{
				position.y = -GlobalFunction::getRandomRange(200, 300);
			}
			position_origin = player_position + position;
			sprite_monster.setPosition(position_origin);
			Health_maintenance();
			speed = speed_initial;
			time_clock_destination = GlobalFunction::getRandomRangeIfFloat(15, 35) / 10;
			is_moving = true;
			Get_angle(player_position, velocity_player);
		}
	}
	...
	if (!being_pushed)
	{
		if (!monster_is_stunned)
		{
			if (is_moving)
			{
				...
				speed += speed_initial*0.01;

				if (GlobalFunction::GetDistanceBetweenTwoPositions(position_origin, GetCurrentPosition()) >= distance_traveling)
				{
					MonsterReachedDestination();
				}
			}
...
void Bomby::Get_angle(sf::Vector2f player_position, sf::Vector2f velocity_player)
{
	sf::Vector2f monster_position = GetCurrentPosition();
	float angle = GlobalFunction::GetRotationBetweenTwoPositions(monster_position, player_position);
	sf::Vector2f position_aiming = GlobalFunction::PositionPrediction(monster_position, player_position, velocity_player, speed);
	float angle_aiming = GlobalFunction::GetRotationBetweenTwoPositions(monster_position, position_aiming);

	
	position_going_to = position_aiming + sf::Vector2f(-(cos(angle_aiming * PI / 180))*50, -(sin(angle_aiming * PI / 180))*50);
	angle_going = angle_aiming;
}

void Bomby::Cutting_sprite(sf::Vector2f player_Position)
{
	if (clock_animation.getElapsedTime().asSeconds() >= 0.05)
	{
		sf::Vector2f current_position = GetCurrentPosition();
		float distance_moved = GlobalFunction::GetDistanceBetweenTwoPositions(current_position, position_origin);
		float percent_distance_moved = distance_moved / distance_traveling;

		source_x = std::floor(percent_distance_moved * 5);
		if (source_x >= 5)
		{
			source_x == 4;
		}

		sprite_monster.setTextureRect(sf::IntRect(monster_size.x * source_x, type_bomby * monster_size.y, monster_size.x, monster_size.y));
		shadow.setTextureRect(sf::IntRect(monster_size.x * source_x, type_bomby * monster_size.y, monster_size.x, monster_size.y));
	
		clock_animation.restart();
	}
}

sf::Vector2f Bomby::GetCurrentPosition()
{
	return sprite_monster.getPosition();
}

sf::Vector2f Bomby::GetSize()
{
	return monster_size;
}
...
float Bomby::GotDamaged(float damage_dealt, int id_attacker, float armor_penetration_percent)
{
...
float Bomby::GetDamage()
{
	return damage;
}

void Bomby::Update(sf::Vector2f playerPosition, float player_speed, sf::Vector2f player_position, float DELTATIME, sf::Vector2f velocity_player)
{
	StunGestion();
	Health_maintenance();
	Movement_gestion(DELTATIME, player_position, velocity_player);
...
sf::String Bomby::GetMonsterType()
{
	return "Bomby";
}
...
sf::Vector2f Bomby::GetTypeAndNumberFragments()
{
	return sf::Vector2f(monster_type_fragment_given, monster_fragments_given);
}

void Bomby::GetIgnited(int type_ignition, float duration_ignition, float damage_ignition)
{
...
float Bomby::DamageFromAllIgnition()
{
...
void Bomby::IgnitionAnimationGestion()
{
...
void Bomby::GetSlowed(int type_slow, float duration_slow, float percent_slow)
{
...
void Bomby::CalculateMovementSpeedWithSlows()
{
...
float Bomby::GetStunTime()
{
	return time_being_stunned;
}

void Bomby::StunBarGestion()
{
	if (monster_is_stunned)
	{
...
void Bomby::ChangeStunTime(float stun_time_received)
{
	if (is_moving)
	{
...
void Bomby::GetPushedAway(float distance_being_pushed_received, int force_of_pushing, sf::Vector2f origin_of_propulsion_receveid, sf::Vector2f origin_object_blocking)
{
...
void Bomby::Draw(sf::RenderWindow &window)
{
	if (is_moving)
	{
...
void Bomby::DrawShadow(sf::RenderWindow &window)
{
	if (is_moving)
	{
...
bool Bomby::CheckIdObject(int id_object)
{
...
void Bomby::DealWithCollision(std::shared_ptr<CollisionalObject> object_collided)
{
	...
	if (CheckIdObject(id_object))
	{
		...
		if (type_object == Player_E)
		{
			...
			if (type_bomby == DamageBomby)
			{
				object_collided->GotDamaged(GetDamage(), id_monster, 0);
			}
			else if (type_bomby == PoisonBomby)
			{
				object_collided->GivePlayerChangeStat(HpRecoveryPercentIncrease, 5, -40);
			}
			else
			{
				object_collided->GivePlayerChangeStat(MovementSpeedPercent, 2, -25);
			}
		}
	}
}

void Bomby::PutItBackInQuadtree()
{
	need_to_be_put_in_quadtree = true;
}

bool Bomby::CheckIfNeedGoBackQuadtree()
{
...
void Bomby::HealObject(float heal_value)
{
	if (!listEliteBonus[DividedHealth_E])
	{
...
void Bomby::ChangeBombyAspect()
{
	type_bomby = GlobalFunction::getRandomRange(1, 3) - 1;
}

void Bomby::MonsterReachedDestination()
{
	speed = speed_initial;
	is_moving = false;
