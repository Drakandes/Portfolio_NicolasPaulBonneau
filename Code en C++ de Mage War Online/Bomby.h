...
class Bomby : public MonsterBase, public std::enable_shared_from_this<Bomby>
{
private:
	enum BombyAspect{DamageBomby,PoisonBomby,IceBomby};
	...
	sf::Vector2f monster_size{ 30,29 };
	sf::Vector2f position_going_to;
	sf::Vector2f position_origin;
	float speed_initial = 500;
	float speed;
	sf::Clock clock_blob_update;
	float monster_maximum_health = 104;
	float armor = 12;
	float reset_timer = 0.5;
	float attack_speed = 1;
	float monster_holder_health;
	float damage = 165;
	float time_clock_destination = 1.75;
	float distance_traveling = 600;
	...
	sf::Clock clock_switch_destination;
	int type_bomby = DamageBomby;
	int source_x = 0;
	int range_of_view = range_of_view_monster;// 600;
	int rayon = 30;
	int level_monster = 1;
	...
	void ChangeBombyAspect();
	void MonsterReachedDestination();
public:
	Bomby();
	~Bomby();
	...
	void Movement_gestion(float DELTATIME, sf::Vector2f player_position, sf::Vector2f velocity_player);
	void Get_angle(sf::Vector2f player_Position, sf::Vector2f velocity_player);
